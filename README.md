...
Built for Agent Forge SG on a unified agentic stack: **Bright Data** (live web), **Kimi K2** (reasoning), **Daytona** (sandboxed compute), **VideoDB** (earnings-video perception), **Terminal 3** (verifiable agent identity + mandate). Each integration runs live when its key is present and falls back to cached data otherwise, so the demo never breaks.
